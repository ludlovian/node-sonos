 ...
 */
  ...
  async cancelAllSubscriptions () {
    ...
    const clearRenewTimer = function () {
      if (self._renewTimer) clearInterval(self._renewTimer)
      self._renewTimer = undefined
    }
    const cancelAll = Object.keys(this.subscriptions).map(cancel)
    return Promise.all(cancelAll).then(clearRenewTimer)
  }
